...
class DockerFile(GeneratedTemplateFile):
    ...
    def __init__(self, experiment, requirements_file):
        ...
        experiment = attr.evolve(
            experiment, cmd=StaticCmd(command=updated_cmd, env=experiment.env)
        )
...
class DockerEngine(object):
    ...
    def _login_with_docker(self, experiment):
        self._client.login(
            registry=experiment.registry_url,
            username=experiment.registry_username,
            password=experiment.registry_password,
            reauth=True,
        )
    ...
    def build_and_publish_image(self, experiment):
        ...
        if registry_url:
            _login = (
                self._login_with_gcloud if self._is_gcr else self._login_with_docker
            )
            _login(experiment)
        ...
        dockerfile = DockerFile(
            experiment=experiment, requirements_file=requirements.path
        )
    ...
    def _generate_requirements_name(self, experiment):
        return "requirements_{}_{}.txt".format(
            experiment.project.replace("/", "_"), experiment.name
        )
    ...
    def _generate_repository_name(self, experiment):
        ...
        if experiment.registry_url:
            ...
            if self._is_gcr:
                assert (
                    experiment.google_project_id
                ), "Configure google_project_id key for current context"
