...
@click.option(
    "--storage",
    default=None,
    help="Storage path to which neptune will copy source code",
)
@click.option(
    "--resources",
    default=None,
...
def context_add(ctx, name, backend_type, storage, resources, registry_url, neptune):
    ...
    if set(resources) - set(AVAILABLE_RESOURCES):
        ...
        raise click.ClickException(
            "Unknown resource type: {}".format(",".join(unknown_resources))
        )
    context = {
        "context_name": name,
...
def context_edit(ctx, name):
    ...
    try:
        ...
        if updated_text:
            updated_context = (
                updated_text and yaml.load(StringIO(updated_text)) or context
            )
